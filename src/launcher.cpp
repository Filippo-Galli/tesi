...
RCPP_MODULE(params_module) {
    Rcpp::class_<Params>("Params")
        .constructor<double, double, double, double, double, double, int, int, double, double, double, Eigen::MatrixXd>(
    ...
    Rcpp::class_<Covariates>("Covariates")
        .constructor<Eigen::MatrixXi, double, Eigen::VectorXi, double, double, double, bool, double, double>(
            "Constructor covariates with all parameters")
        .constructor("Default constructor")
        .field("W", &Covariates::W, "Adjacency matrix defining spatial neighborhood structure")
        .field("spatial_coefficient", &Covariates::spatial_coefficient,
        ...
        .field("fixed_v", &Covariates::fixed_v, "Flag indicating if variance v is fixed")
        .field("nu", &Covariates::nu, "Degrees of freedom for the variance prior")
        .field("S0", &Covariates::S0, "Scale parameter for the variance prior");
}
 ...
 */
// [[Rcpp::export]]
Rcpp::List mcmc(Params &param, Covariates &covariates,
    ...
    // SplitMerge_LSS sampler(data, param, likelihood, process, true); // Split-Merge with SAMS using sequential
    // allocation
    ...
    std::cout << "Starting MCMC with " << param.NI << " iterations after " << param.BI << " burn-in iterations."
              << std::endl;
    ...
    std::cout << "MCMC completed in : " << std::chrono::duration_cast<std::chrono::seconds>(end - begin).count()
              << " seconds." << std::endl;
    std::cout << "U acceptance rate: " << U_sampler.get_acceptance_rate() * 100 << " %." << std::endl;

    std::cout << "Accepted split ratio: " << sampler.get_accepted_split() * 100 * 2 / (param.NI + param.BI) << " %."
              << std::endl;
    std::cout << "Accepted merge ratio: " << sampler.get_accepted_merge() * 100 * 2 / (param.NI + param.BI) << " %."
              << std::endl;
    std::cout << "Accepted shuffle ratio: " << sampler.get_accepted_shuffle() * 100 / (param.NI + param.BI) << " %."
              << std::endl;
