/**
 * @file launcher.cpp
 * @brief Main launcher file for MCMC simulation with various Bayesian non-parametric processes
 * @author Filippo Galli
 * @date 2025
 * 
 * This file contains the main MCMC implementation and R interface for running
 * Bayesian non-parametric clustering using different processes (DP, DPW, NGGP, NGGPW)
 * with various sampling strategies (Neal3, Split-Merge, Split-Merge SAMS).
 */
 ...
 * The module exposes constructors and all parameter fields with their descriptions.
 */
 ...
 * 
 * This function performs Markov Chain Monte Carlo (MCMC) sampling for clustering
 * analysis using various Bayesian non-parametric processes. It supports different
 * process types (DP, DPW, NGGP, NGGPW) and sampling algorithms (Neal3, Split-Merge).
 * 
 * @param distances A matrix of distances between data points (Eigen::MatrixXd)
 * @param param Reference to a Params object containing all MCMC parameters and hyperparameters
 * @param initial_allocations_r Optional initial cluster allocations (Rcpp::IntegerVector).
 *                             If empty, random initialization will be used.
 * 
...
mcmc(const Eigen::MatrixXd &distances, Params &param,
  ...
  // Convert R IntegerVector to Eigen::VectorXi for compatibility with Eigen-based classes
