...
class NGGPSplitMergeWSAMSMartinezMena : public Sampler {
  private:
    ...
    void shuffle();
    double compute_acceptance_ratio_shuffle(double likelihood_old_ci, double likelihood_old_cj, int old_ci_neighbors, int old_cj_neighbors,
                                            int old_ci_size, int old_cj_size);

    int cluster_neighbors(int cluster);

  public:
    NGGPSplitMergeWSAMSMartinezMena(Data &d, Params &p, Likelihood &l, bool update_tau = false, double alpha = 0.1, double beta = 0.1)
