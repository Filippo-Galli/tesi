...
void NGGPNeal2::step_1_observation(int index) {
    /**
     * @brief Performs a step in the NGGPNeal2 sampling process.
    ...
    */
    ...
    for (int k = 0; k < data.get_K(); ++k) {
        log_likelihoods[k] += log(data.get_cluster_size(k) - params.sigma);
    }
...
double NGGPNeal2::log_conditional_density_V(double v) const {
    /**
...
double NGGPNeal2::log_conditional_density_W(double w) const {
    /**
