...
/**
 * @class Likelihood
 * @brief Computes log-likelihood for clusters based on distance-based cohesion and repulsion
 * 
 * This class implements a likelihood model that encourages points within clusters
 * to be close (cohesion) while clusters are pushed apart (repulsion). The model
 * uses gamma priors on distance distributions.
 */
class Likelihood {
private:
    ...
    double compute_cohesion(int point_index, int cluster_index, 
                           const Eigen::VectorXi cls_ass_k, int n_k) const;
    
    /**
     * @brief Computes the repulsion component of the likelihood for a point
     * @details Calculates the repulsive effect between a point and all other clusters
     *          using inter-cluster distances with a gamma likelihood
     * @param point_index Index of the point being evaluated
...
public:
     ...
     * @details Computes both cohesion (within-cluster) and repulsion (between-cluster)
     *          components for all points in the specified cluster
    ...
    /**
     * @brief Calculates the full log-likelihood of a cluster with explicit assignments
    ...
    /**
     * @brief Calculates the conditional log-likelihood of assigning a point to a cluster
     * @details Computes how likely a point is to belong to a cluster, considering
     *          both its cohesion with the cluster and repulsion from other clusters
    ...
    /**
     * @brief Calculates the conditional log-likelihood with explicit cluster assignments
     * @details Overloaded version that accepts cluster assignments directly
     * @param point_index Index of the point to evaluate
     * @param cluster_index Index of the target cluster
     * @param cls_ass_k Vector of point indices in the cluster
     * @return Conditional log-likelihood of the point given the cluster
     */
    double point_loglikelihood_cond(int point_index, int cluster_index, 
